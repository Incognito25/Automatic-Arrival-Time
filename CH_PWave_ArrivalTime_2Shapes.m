...
ResultMatrix = [string('Depth of Source(m)'),string('Depth of Receiver(m)'), string('P-Wave File Name'), string('Window Width'), string('R2'), string('Arrival Time (ms)')];
...
[num, ~, ~] = xlsread('CH E59 Data Sheet', 'Data Sheet');
...
PWaveFileName = 'ST'+PWaveFileName;
         ...
         signal (1,:) = signal(1,:)/1000000; %Decreases the value of delay peak
...
ArrivalTime;
pause(1);
plotname = strcat('CH_',PWaveFileName,'_CURVE_FIT_',num2str(WindowWidth));
...
str = {['File Name = ',num2str(PWaveFileName)],['Arrival Time = ',num2str(ArrivalTime),' ms'],['Window Width = ',num2str(WindowWidth)],['R^{2} = ',num2str(R_sqr_selected)]};
annotation('textbox',dim,'String',str,'FitBoxToText','on');
plotname = strcat('CH_',PWaveFileName,'_ARRIVAL_TIME_',num2str(WindowWidth));
print(gcf, '-dtiff', '-r600', plotname)

close all

 ResultMatrix = [ResultMatrix; [DepthOfSource DepthOfReceiver PWaveFileName WindowWidth R_sqr_selected ArrivalTime]]
end
ResultMatrix;